...
def torser(update, context):
    ...
    try:
        ...
        return sendMessage("Ohk Search , But What? Atleast Give A Name For Searching In #Torrent Sites", context.bot, update)
    if SEARCH_API_LINK is not None and SEARCH_PLUGINS is not None:
        ...
        sendMarkup('<b>Choose site to search</b>: \n\n<b>I Recommend</b> \n\n<b>#Piratebay For Search Accuracy</b>,\n\n<b>#Rarbg For High Quality</b>', context.bot, update, button)
...
def torserbut(update, context):
    ...
    if user_id != int(data[1]):
        ...
        query.answer()
        button = api_buttons(user_id)
        editMessage('<b>Choose site to search</b>: \n\n<b>I Recommend</b> \n\n<b>#Piratebay For Search Accuracy</b>,\n\n<b>#Rarbg For High Quality</b>', message, button)
    elif data[2] == 'plugin':
        query.answer()
        button = plugin_buttons(user_id)
        editMessage('<b>Choose site to search</b>: \n\n<b>I Recommend</b> \n\n<b>#Piratebay For Search Accuracy</b>,\n\n<b>#Rarbg For High Quality</b>', message, button)
        ...
        tool = data[3]
        if tool == 'api':
            editMessage(f"Searching for <b>#{key}</b>\nTorrent Site:- <b>#{SITES.get(site)}</b>", message)
        else:
            editMessage(f"Searching for <b>#{key}</b>\nTorrent Site:- <b>#{site.capitalize()}</b>", message)
        threading.Thread(target=search, args=(key, site, message, tool)).start()
    else:
        query.answer()
        editMessage("#Torrent Search has been canceled!", message)
    ...
    if tool == 'api':
        ...
        try:
            ...
            if isinstance(search_results, list):
                msg = f"Found <b>{min(len(search_results), SEARCH_LIMIT)}</b>"
                msg += f"Result for <b>#{key}</b>\nTorrent Site:- <b>#{SITES.get(site)}</b>"
            else:
                return editMessage(f"No result found for <b>#{key}</b> \nTorrent Site:- <b>#{SITES.get(site)}</b>", message)
        ...
        if total_results != 0:
            msg = f"Found <b>{min(total_results, SEARCH_LIMIT)}</b>"
            msg += f" Result for <b>{key}</b>\nTorrent Site:- <b>#{site.capitalize()}</b>"
        else:
            return editMessage(f"No Result found for <b>#{key}</b>\nTorrent Site:- <b>#{site.capitalize()}</b>", message)
...
def getResult(search_results, key, message, tool):
    ...
    path = [telegraph.create_page(
                title='Superior-Mirror Torrent Search',
                content=content
