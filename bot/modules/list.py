...
def list_buttons(update, context):
    ...
    try:
        ...
        return sendMessage('Send Atleast Filename with the command.', context.bot, update)
...
def select_type(update, context):
    ...
    if user_id != int(data[1]):
        ...
        list_method = data[3]
        item_type = data[2]
        editMessage(f"𝐒𝐞𝐚𝐫𝐜𝐡𝐢𝐧𝐠 𝐒𝐢𝐫.. 𝐖𝐚𝐢𝐦𝐭 {key}", msg)
...
def list_drive(key, bmsg, list_method, item_type):
    ...
    if button:
        ...
        editMessage(f"<b>Seems #{key} Is Not In My Drive\n\nBut Don't Worry You can still search from torrent \n\nDo /searchx File Name</b>", bmsg)
